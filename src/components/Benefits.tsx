/* Scannable "what you get" row near the top — fast benefit payoff,
   the lesson borrowed from Meridian, in Atrium's calm operator voice. */
...
const BENEFITS = [
  ...
  },
  {
    ...
    ico: <g><rect x="2" y="6" width="20" height="13" rx="2" /><circle cx="12" cy="12.5" r="3" /><path d="M6 10v0M18 15v0" /></g>,
  },
];

export default function Benefits() {
  return (
    <section className="ben" id="elonyok">
      <div className="wrap">
        <div className="dash__intro reveal">
          <span className="dash__eyebrow">Miért éri meg</span>
          <h2 className="dash__h">Amit a rendszer Önnek hoz.</h2>
        </div>
        <div className="ben__grid reveal" data-delay="1">
          {BENEFITS.map((b, i) => (
            <div className="ben__item" key={i}>
              <span className="ben__ico">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
                  {b.ico}
                </svg>
              </span>
              <b className="ben__t">{b.t}</b>
              <span className="ben__d">{b.d}</span>
            </div>
          ))}
        </div>
      </div>
