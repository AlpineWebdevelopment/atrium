/* ATRIUM-EDIT FG6/FG7 — dental full system, 3 phases.
   ...
   CHANNELS: Telefon · SMS · E-mail (3 only — dental is phone-led; FG6)
   FG5 framing: "nem szűrünk" carried in Phase 01 step copy (not separate element)
...
/* FG6 — 3 phases, dental-native steps */
const PHASES: {
...
}[] = [
  {
    ...
    steps: [
      { ic: "phone",   t: "Hívásfogadás",            s: "minden hívást fogad: zárás után, hétvégén, amíg Ön egy páciens székénél áll — nyugodt, magyar beszéd, mert a hívó gyakran fájdalmakkal küzd" },
      { ic: "triage",  t: "Sürgősségi rangsorolás",  s: "akut fájdalom vagy rutin? A sürgőset a rendelő protokollja szerint eszkalálja, a rutint lefoglalja — senkit nem szűr ki, mindenkit a helyére irányít" },
      { ic: "chat",    t: "Kérdéskezelés",            s: "nyitvatartás, beutaló kell-e, kell-e időpont, nagyjából mennyi egy kontroll vagy egy fogkőeltávolítás — a pult nem fullad bele" },
    ],
  },
  {
    n: "02", name: "Foglalás és emlékeztetés", cap: "időpont, megerősítve, megtartva", c: "#628FBC", Viz: VizFoglalas,
    steps: [
      { ic: "calendar", t: "Foglalás",                      s: "a rendelő meglévő naptárába, a páciensnek visszaigazolva" },
      { ic: "bell",     t: "Emlékeztető",                   s: "az időpont előtt, kevesebb meg nem jelenés" },
      { ic: "callback", t: "Elmaradt időpont visszahívása", s: "a kihagyott kezelésre újrafoglalási megkeresés" },
    ],
  },
  {
    n: "03", name: "A bázis megtartása", cap: "visszahívva, mielőtt elsodródna", c: "#2D9B7F", Viz: VizBazis,
    steps: [
      /* ATRIUM-EDIT FG6 — Visszahívás esedékességkor gets extra visual weight: .tsr__step--lead */
      { ic: "refresh", t: "Visszahívás esedékességkor", s: "a rendszer figyeli a bázist, és szól, ha egy páciens kontrollja vagy higiéniai kezelése esedékes — ez hozza vissza az elsodródott pácienseket: saját, meglévő bevétel, nem új ügyfélszerzés", lead: true },
      { ic: "star",    t: "Értékeléskérés",             s: "kezelés után" },
      /* ATRIUM-EDIT FG6 — Riport gets extra visual weight: .tsr__step--lead */
...
/* FG6 — 3 channels: Telefon · SMS · E-mail */
...
/* FG6 — 6 dental-native capabilities */

export default function FullSystemFogaszat() {
  return (
      ...
      <div className="wrap">
        <div className="dash__intro reveal">
          ...
          <p className="dash__p">
            ...
            fogászati rendelőknek: minden hívást fogad és rangsorol, lefoglalja
            a rutin időpontokat, és visszahívja a pácienseket, mielőtt
            lemaradnának a kontrollról — a háttérben, a meglévő naptárához kötve.
          </p>
          {/* ATRIUM-EDIT FG7 — AI named honestly; CRM mention; disclosure sentence */}
              ...
              <div className={"tsr__row reveal" + (i % 2 ? " tsr__row--flip" : "")} key={i} style={{ ["--c" as string]: p.c } as React.CSSProperties}>
                ...
                <div className="tsr__body">
                  ...
                  <ul className="tsr__steps">
                    {p.steps.map((st, j) => (
                      /* ATRIUM-EDIT FG6 — lead steps (Visszahívás + Riport) get .tsr__step--lead visual weight */
