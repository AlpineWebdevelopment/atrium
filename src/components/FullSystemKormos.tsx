/* Nail / körmös full system — coral phase palette.
   De-fabbed live stat, GoHighLevel removed, mester→körmös applied. */
...
const CAPS = [
  { ic: "mic", c: "#C4705A", t: "Hang-AI", d: "Természetes magyar beszéd — fogadja a hívásokat és visszahív." },
  { ic: "chat", c: "#628FBC", t: "Szöveges AI", d: "SMS, WhatsApp, Messenger, Instagram, e-mail és webchat — egy helyen." },
  { ic: "send", c: "#BCA162", t: "Proaktív megkeresés", d: "Nem csak válaszol — magától keresi meg a régi és kihűlt vendégeket." },
  { ic: "db", c: "#62BCAC", t: "Egységes vendég-memória", d: "Minden csatornán emlékszik a vendégre és az előző kezelésre." },
  { ic: "qualify", c: "#628FBC", t: "Kvalifikáció", d: "Felteszi a fontos kérdéseket: kezelés típusa, körmös, időpont." },
  { ic: "refresh", c: "#6DBC61", t: "Naptár- és CRM-szinkron", d: "A meglévő eszközeihez kötve, valós időben." },
  { ic: "bars", c: "#BCA162", t: "Riport", d: "Havi kimutatás: teli naptár, no-show arány, bevétel." },
  { ic: "clock", c: "#62BCAC", t: "0–24 elérhetőség", d: "Kezelés közben, zárás után, hétvégén — mindig felveszi." },
];
...
function VizMegkereses() {
  ...
  return (
    <svg className="tsr__svg" viewBox="0 0 300 210" role="img" aria-label="Megkeresés — minden hívás egy foglalássá">
      {ch.map((c, i) => {
      ...
      <text className="tsr__viz-lbl" x="226" y="160" textAnchor="middle">szűrve, foglalva</text>
    </svg>
  );
}
...
function VizMegtartas() {
  ...
  return (
    <svg className="tsr__svg" viewBox="0 0 300 210" role="img" aria-label="Megtartás — a vendég újra és újra visszatér">
...
const PHASES = [
  {
    n: "01", name: "Megkeresés", cap: "az első hívástól a foglalásig", c: "#C4705A", Viz: VizMegkereses,
    steps: [
      { ic: "phone", t: "Hívásfogadás", s: "minden hívást felvesz, kezelés közben és zárás után is" },
      { ic: "chat", t: "Azonnali válasz", s: "Instagramon, webchat-en, SMS-en percek alatt visszaír" },
      { ic: "qualify", t: "Szolgáltatás szűrés", s: "felteszi a fontos kérdéseket: kezelés, körmös, időpont" },
      { ic: "handoff", t: "Élő átadás", s: "ha személyes döntés kell, azonnal kapcsol" },
    ],
  },
  {
    n: "02", name: "Foglalás", cap: "időpont, megerősítve", c: "#BCA162", Viz: VizFoglalas,
    steps: [
      { ic: "calendar", t: "Időpontfoglalás", s: "egyenesen a naptárba, a megfelelő körmöshöz" },
      { ic: "confirm", t: "Visszaigazolás", s: "azonnal visszaigazolja a foglalást" },
      { ic: "bell", t: "Emlékeztető", s: "24 és 2 órával előtte értesíti a vendéget" },
      { ic: "reschedule", t: "Várólistakezelés", s: "lemondás esetén azonnal értesíti a várólistán lévőket" },
    ],
  },
  {
    n: "03", name: "Megtartás", cap: "vendégek visszahozva", c: "#62BCAC", Viz: VizMegtartas,
    steps: [
      { ic: "callback", t: "Elmaradtak visszahívása", s: "visszaszerzi az elmaradt kezelést" },
      { ic: "star", t: "Értékelés", s: "elégedett vendégtől Google-értékelést kér" },
      { ic: "refresh", t: "Reaktiválás", s: "régen nem járt vendégeket szezonális ajánlattal visszahívja" },
      { ic: "users", t: "Ajánláskérés", s: "az elégedett vendégtől ajánlást is kér — új vendég a meglévőből" },
    ],
  },
];
...
const CHANNELS: { n: string; c: string; ic: React.ReactNode }[] = [
  ...
  { n: "Instagram", c: "#C4705A", ic: <g><rect x="3" y="3" width="18" height="18" rx="5" /><circle cx="12" cy="12" r="4" /><path d="M17.5 6.5h.01" /></g> },
  ...
  { n: "E-mail", c: "#BCA162", ic: <g><rect x="3" y="5" width="18" height="14" rx="2" /><path d="M3 7l9 6 9-6" /></g> },
    ...
    <section className="sys" id="rendszer-teljes">
      <div className="wrap">
        <div className="dash__intro reveal">
          ...
          <p className="dash__p">
            Az Atrium egy magyar nyelvű, AI-alapú rendszer
            körmösöknek és műkörömépítőknek: minden csatornán ugyanaz a közös memória
            dolgozik, és végigkíséri a vendéget az első hívástól a
            visszatérő törzsvendéggé válásig — a háttérben, az Ön
            meglévő naptárához kötve.
          </p>
          <p className="sys__howline">
            Hogyan működik? AI-ügynökök kezelik a hang- és szöveges
            beszélgetést, természetes magyar nyelven — a csapatának
            semmit nem kell lecserélnie.
          </p>
        </div>
        ...
        <div className="sysframe reveal" data-delay="1">
          <div className="sysframe__bar">
            <div className="sysframe__bar-top">
              <span className="sysframe__label">
                ...
                Az Atrium rendszer · vendégek egységes memóriával
              </span>
              {/* ATRIUM-EDIT NK-SD — de-fabbed: fabricated counter replaced with channel-agnostic live label */}
          ...
          <div className="tsr">
            {PHASES.map((p, i) => (
              <div className={"tsr__row reveal" + (i % 2 ? " tsr__row--flip" : "")} key={i} style={{ ["--c" as string]: p.c } as React.CSSProperties}>
                <div className="tsr__viz"><p.Viz /></div>
          ...
          <div className="jrn__result reveal">
            ...
            <div className="jrn__result-txt">
              <b className="jrn__result-t">Teli naptár, több bevétel</b>
              <span className="jrn__result-s">és minden lépés mérve — riportálás</span>
            </div>
          </div>
          {/* ATRIUM-EDIT NK-SD2 — GoHighLevel removed; brand spec §10 prohibits internal stack on public surfaces */}
          <div className="sysframe__integ">
            Csatlakozik a meglévő eszközeihez: <b>Google Calendar</b> · <b>Microsoft 365</b> · <b>iCal</b> · <b>a meglévő CRM-jéhez</b>
          </div>
        </div>

        <div className="caps reveal" data-delay="2">
          <div className="caps__head"><h3 className="caps__h">Mit tud a rendszer?</h3></div>
