/* "Mit csinálunk" — two clearly separated layers.

   The base is the ChatGPT-ads acquisition system that every engagement
   includes: question research, competitor analysis, offer, creative,
   campaign management, landing page, CRM, and monthly reporting.

   The AI automation on top of it (callback in minutes, pre-qualification,
   booking, follow-up) is separate work. We do build it, but it is not part
   of the base and it is scoped in the meeting — the page must not imply it
   comes with the campaign, and it must not read as a package either. */
...
const ICONS: Record<string, React.ReactNode> = {
  ...
  crm: <g><rect x="3.5" y="4.5" width="17" height="15" rx="2" /><path d="M9 4.5v15" /><path d="M12 9h6M12 12.5h6M12 16h3.5" /></g>,
...
const BASE = [
  ...
  { ic: "crm",      c: "var(--t-slate)", t: "CRM",                        d: "Minden érdeklődő egy helyre kerül, forrással együtt. Látja, ki honnan jött, mikor, és mi lett vele. Az adat az Öné." },
    ...
    <section className="sys cg-sys" id="rendszer-teljes">
      <div className="wrap">
        <div className="dash__intro reveal">
          ...
          <p className="dash__p">
            A hirdetés csak az első lépés. Ezért nemcsak a kampányt visszük, hanem azt is, ahova a kattintás érkezik, és azt is, amiből havonta látja, mi lett belőle.
          </p>
        </div>

        <div className="cg-tiles reveal" data-delay="1">
          {BASE.map((c, i) => (
            <div className="cg-tile" style={{ ["--c" as string]: c.c }} key={c.t}>
        ...
        {/* the automation layer, deliberately set apart from the base */}
        <div className="cg-extra reveal" data-delay="2">
          ...
          <div className="cg-extra__grid">
            {EXTRA.map((e) => (
              <div className="cg-extra__item" key={e.t}>
                <span className="cg-extra__ico">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{ICONS[e.ic]}</svg>
                </span>
                <span className="cg-extra__t">{e.t}</span>
        ...
        <p className="sys__howline reveal cg-howline" data-delay="3">
          AI növekedési partnerként dolgozunk: ahol egy folyamat AI-val jobban megy, ott AI-val csináljuk. Ahol nem, ott nem. A cél nem az AI, hanem az új ügyfél.
        </p>

        <div className="sec-cta reveal" data-delay="4">
