/* The one named product: the AI qualifier CRM — physio edition. */

function GfxTuned() {
  const sizes = [
    { lbl: "1 terapeuta", h: 32 },
    { lbl: "2–4 terapeuta", h: 56 },
    { lbl: "5+ terapeuta", h: 80 },
  ];
  ...
  return (
    <svg viewBox="0 0 260 160" role="img" aria-label="Minden rendelőméretre van megoldásunk">
      <text className="qben__lbl" x={130} y={16} textAnchor="middle"
        style={{ fill: "var(--acc)", fontSize: "9.5px", letterSpacing: "0.06em" }}>MINDEN RENDELŐMÉRETRE</text>
      {sizes.map((t, i) => {
        const cx = centers[i];
...
export default function QualifierCrmPhysio() {
  return (
      ...
      <div className="wrap">
        <div className="dash__intro reveal">
          ...
          <p className="dash__p">
            ...
            kérdéseket, minősíti a pácienst, és csak azzal foglal kezelési időpontot,
            akinek tényleg szüksége van a terápiára.
          </p>
        </div>
