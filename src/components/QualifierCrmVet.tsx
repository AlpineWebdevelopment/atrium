/* The one named product: the AI qualifier CRM — vet edition.
   Four reasons it fits — each its own panel with a bespoke graph. */

/* ---- 1: Rendelőjére szabva — staircase: clinic sizes, each with a solution ---- */
function GfxTuned() {
  const tiers = [
    { lbl: "1 orvos", h: 32 },
    { lbl: "2–4 orvos", h: 56 },
  ...
  return (
    <svg viewBox="0 0 260 160" role="img" aria-label="Minden rendelőméretre van megoldásunk">
      <text className="qben__lbl" x={130} y={16} textAnchor="middle"
        style={{ fill: "var(--acc)", fontSize: "9.5px", letterSpacing: "0.06em" }}>1 ORVOS · SAJÁT PROTOKOLL</text>
      {tiers.map((t, i) => {
        const cx = centers[i];
...
/* ---- 3: Több vizsgálat — same 8 leads, 3 vs 8 appointments ---- */
function GfxFunnels() {
  return (
